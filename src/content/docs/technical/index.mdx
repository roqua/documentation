...
Access to these API endpoints is controlled by [API tokens](/docs/admin/integration/api_tokens/) in [RoQua-Admin](https://rom.roqua.nl/manage). An administrator might generate an API token with rights to (a) certain endpoint(s) and provide the developer with a `consumer_key` and `consumer_secret`.
...
From [Dossier-specific data / Responses](/technical/rest_api/dossier/responses/):
...
Authentication can also be done using [JWT-token](/technical/authentication/#jwt-tokens). (For testing, you could generate them using [jwt.io](https://jwt.io).)
