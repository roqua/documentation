...
The REST API supports two authentication schemes. Access to the API is controlled by [API tokens](/docs/admin/integration/api_tokens/) in [RoQua-Admin](https://rom.roqua.nl/manage). When opting for JWT tokens, please provide us with your public key so we can hook it to the API token.
...
Create a token via `/admin/api_tokens/new` [(manual)](/docs/admin/integration/api_tokens/). After creation you receive:
